/*
Description: Trajectory generator
Email: 13247344844@163.com
Author: Shihao Feng
Update time: 2022-11-03
*/

// TODO: 简化，删除不必要的东西

#ifndef TRAJ_GENERATOR_H
#define TRAJ_GENERATOR_H

#include <iostream>
#include <vector>
using namespace std;
#include <math.h>
#include <eigen3/Eigen/Dense> 
using namespace Eigen;
#include "pinocchio/algorithm/kinematics.hpp" 
#include "pinocchio/spatial/se3.hpp"
namespace pin = pinocchio;


class TimeScale
{
public:
    // TimeScale();
    Vector3d first_order_poly(const double &s);
    Vector3d third_order_poly(const double &T, const double &s);
    Vector3d fifth_order_poly(const double &T, const double &s);
    Vector3d quick_foot_fall(const double &T, const double &s);
    // ~TimeScale();

    inline bool check(const double &s)
    {
        if (s >= 0. && s <= 1.) {return true;}
        cout << "[Time Scale Error]: s should belong to [0,1]\n";
        return false;
    }
};


class Point2PointTrajGenerator
{
public:
    // Point2PointTrajGenerator();
    vector<Vector3d> linear_traj(const Vector3d &q_start, 
                                const Vector3d &q_end, 
                                const double &T, 
                                const double &s,
                                int order = 5); 

    pin::SE3 linear_decoupled_traj(const pin::SE3 &T_start, 
                                const pin::SE3 &T_end, 
                                const double &T,
                                const double &s, 
                                int order = 5); 

    pin::SE3 linear_coupled_traj(const pin::SE3 &T_start, 
                                const pin::SE3 &T_end, 
                                const double &T,
                                const double &s, 
                                int order = 5); 

    // ~Point2PointTrajGenerator();

private:
    TimeScale _time_scale;
};



class CycloidTrajGenerator
{
public:
    // CycloidTrajGenerator();
    vector<Vector3d> sw_traj(const Vector3d &p_hf_start, 
                            const Vector3d &p_hf_end, 
                            const double &h_gc, 
                            const double &T_sw, 
                            const double &s_sw);

    vector<Vector3d> st_traj(const Vector3d &p_hf_start, 
                            const Vector3d &p_hf_end, 
                            const double &T_st, 
                            const double &s_st);
    // ~CycloidTrajGenerator();

private:
    TimeScale _time_scale;
};



class BezierTrajGenerator
{
public:
    // BezierTrajGenerator();
    vector<Vector3d> sw_traj(const Vector3d &p_hf_start, 
                            const Vector3d &p_hf_end, 
                            const double &h_gc, 
                            const double &T_sw, 
                            const double &s_sw);

    // vector<Vector3d> st_traj(const Vector3d &p_hf_start, 
    //                         const Vector3d &p_hf_end, 
    //                         const double &T_st, 
    //                         const double &s_st);
    // ~BezierTrajGenerator();

private:
    TimeScale _time_scale;

    double _bernstein_ploy(const int &k, const int &n, const double &s);
    
    vector<Vector3d> _bezier_traj(const MatrixX3d &ctrl_points, 
                        const double &T, 
                        const double &s);

    // 返回值不能用int，否则会越界
    inline size_t _factorial(int num) // 递归方法求阶乘
    {
        if (num ==0) {return 1;}
        return num*_factorial(num-1);
    }
};

#endif